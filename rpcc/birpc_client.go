...
import (
	...
	"github.com/cgrates/rpc/rpcc/internal/svc"
)
...
func WithClient(ctx context.Context, c ClientConnector) context.Context {
	return context.WithValue(ctx, clientContextKey{}, c)
}

// ClientValueFromContext retuns the client from context
func ClientValueFromContext(ctx context.Context) ClientConnector {
	c, _ := ctx.Value(clientContextKey{}).(ClientConnector)
	return c
}
...
func (c *BirpcClient) input() {
	...
	defer cancel()
	pending := svc.NewPending(WithClient(ctx, c))
